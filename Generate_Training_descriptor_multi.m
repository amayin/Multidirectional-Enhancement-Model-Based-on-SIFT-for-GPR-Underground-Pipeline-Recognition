...
mat_file_path = 'C:\Users\chen2\OneDrive\Desktop\新建文件夹\mat\';% mat文件路径
DZT_path = 'D:\硕士文件\gprMAX相关\实测数据\DZT实验数据整合';% DZT文件路径
lte_path = 'D:\硕士文件\gprMAX相关\实测数据\lte良好数据整合';% lte文件路径
png_path = 'D:\硕士文件\SIFT提取\实验结果\与YOLO对比\仿真数据对比\测试集\test_image_png';
output_path = 'D:\硕士文件\SIFT提取\实验结果\扩充训练集之后的实验结果\整合DZT特征描述符_RootSIFT\';% 特征描述符输出路径
